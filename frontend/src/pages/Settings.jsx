...
const Settings = () => {
  ...
  const handleChangePassword = async (e) => {
    ...
    if (pwdForm.new_password !== pwdForm.confirm_password) {
      toast.error('New passwords do not match. Please make sure both passwords are the same.');
      return;
    }
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to change password. Please check your current password and try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 h-full overflow-y-auto">
      <div className="mb-6">
        <h2 className="text-lg font-700 text-text-primary">Settings</h2>
        <p className="text-sm text-text-muted">Configure your system preferences</p>
      </div>
          ...
          <div className="k-card">
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className={`k-btn-primary ${loading ? 'opacity-60 cursor-not-allowed' : ''}`}
              >
