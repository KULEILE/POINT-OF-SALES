  ...
  try {
    ...
    const txResult = await client.query(
      ...
      [
        ...
        (parseFloat(amount_paid) || total_amount).toFixed(2), 
    ...
    if (payment_method === 'credit' && customer_id) {
      ...
      await client.query(
        `INSERT INTO credit_transactions (customer_id, transaction_id, transaction_type, amount, previous_balance, new_balance, due_date, created_by) 
      ...
      await client.query(
        `UPDATE customers SET current_balance = $1, updated_at = NOW() 
         WHERE customer_id = $2`,
...
const getAll = async (req, res) => {
  ...
  try {
    let q = `SELECT t.transaction_id, t.receipt_number, t.transaction_date, t.payment_method, t.total_amount, t.status, t.payment_status, t.cashier_name, t.customer_phone, t.is_guest 
             FROM transactions t WHERE 1=1`;
    const params = [];
    
    if (date_from) { 
      params.push(date_from); 
      q += ` AND DATE(t.transaction_date) >= $${params.length}`; 
    }
    if (date_to) { 
      params.push(date_to); 
      q += ` AND DATE(t.transaction_date) <= $${params.length}`; 
    ...
    if (cashier_id) { 
      ...
      q += ` AND t.cashier_id = $${params.length}`; 
    }
    if (payment_method) { 
      params.push(payment_method); 
      q += ` AND t.payment_method = $${params.length}`; 
    }
    if (status) { 
      params.push(status); 
