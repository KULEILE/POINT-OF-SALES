...
const login = async (req, res) => {
  ...
  try {
    ...
    await auditLog(pool, {
      ...
      action_type: 'LOGIN', action_details: 'User logged in',
      affected_table: 'users', affected_record_id: user.user_id
    });
...
const register = async (req, res) => {
  ...
  try {
    ...
    // ← FIXED: fallback to 12 if BCRYPT_ROUNDS is missing
    ...
    const result = await pool.query(
      `INSERT INTO users
        (username, password_hash, full_name, email, phone, role, status, created_by)
    ...
    await auditLog(pool, {
      user_id:           created_by,
      username:          req.user?.username || 'self_register',
      action_type:       'CREATE_USER',
      action_details:    `Created user: ${newUser.username} role: ${newUser.role}`,
      affected_table:    'users',
      affected_record_id: newUser.user_id,
    });
...
const logout = async (req, res) => {
  try {
    await auditLog(pool, {
      ...
      affected_table:    'users',
      affected_record_id: req.user.user_id,
    });
